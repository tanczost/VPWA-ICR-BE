...
export default class ActivityController {
  constructor() {}
  public async changeActivity({ socket, auth }: WsContextContract, userNick: string) {
    if (!auth.user) {
      return
    }
    socket.broadcast.emit('activityAdd', userNick)
    socket.emit('activityAdd', userNick)
  }
  ...
  public async onConnected({ socket, auth, logger }: WsContextContract) {
    ...
    if (userSockets.size === 0) {
      socket.broadcast.emit('user:online', auth.user?.nickName)
    }
  ...
  public async onDisconnected({ socket, auth, logger }: WsContextContract, reason: string) {
    ...
    if (userSockets.size === 0) {
      // notify other users
      socket.broadcast.emit('user:offline', auth.user?.nickName)
    }
